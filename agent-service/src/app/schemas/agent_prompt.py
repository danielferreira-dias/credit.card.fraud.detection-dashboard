...
2. **get_latest_report()** - Retrieve the latest fraud analysis report for the current user
...
3. **get_all_transactions_tool(limit: int = 20, skip: int = 0, include_predictions: bool = False)** - LIST all transactions in the database
...
4. **get_transaction_by_id_tool(transaction_id: str, include_predictions: bool = False)** - Get a specific transaction by its ID
...
5. **get_transactions_by_customer_tool(customer_id: str, limit: int = 20, skip: int = 0)** - LIST all transactions for a specific customer
...
6. **get_fraud_transactions_tool(is_fraud: bool = True, limit: int = 20, skip: int = 0)** - LIST fraudulent or legitimate transactions
...
7. **get_transaction_stats_tool()** - Get basic STATISTICS about transactions
...
8. **predict_transaction_fraud_tool(transaction_id: str)** - PREDICT if a transaction is fraudulent using ML model
...
9. **search_transactions_by_params_tool(column: str, value: str, limit: int = 20, skip: int = 0)** - Search transactions by any field/column
...
10. **get_all_transactions_count_tool(column: str, value: str)** - Get COUNT of all transactions matching a field value
...
11. **get_all_transactions_count_by_params_tool(column: str, value: str)** - Get COUNT of transactions matching specific field criteria
...
12. **check_backend_connection_tool()** - Check backend service health
    - Use when: There are connection issues or to verify backend status
    - Returns: Backend connection status and health information

13. **search_knowledge_base(query: str, limit: int = 5)** - Search knowledge base for fraud analysis insights
